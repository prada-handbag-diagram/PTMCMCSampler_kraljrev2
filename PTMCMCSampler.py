...
class PTSampler(object):
    ...
    def updateChains(
        ...
        if iter % self.thin == 0:
            ...
            # Mkralj: For self._beta, store beta alongside each saved sample 
            # so the batch writer can print the correct beta for that row
    ...
    def sample(
        ...
        model_param_idx=None,
        nameChainTemps=False,
        beta_step=0.0, # Mkralj: per-accept beta increment
        beta_hold_samples=0, #Mkralj: number of saved samples to keep beta fixed
    ):
        """
        ...
        # Mkralj: beta wiring, PT guard, feasibility (no clipping)
        self.beta_step = float(beta_step) # Mkralj: store controls
        self.beta_hold_samples = int(beta_hold_samples)

        # Mrkalj: Allocate beta trace aligned with saved samples
        N_save = int(Niter / thin) + 1
        self._beta = np.zeros(N_save, dtype=float)

        # Mkralj: PT guard, forbid PT (MPI>1) when using MSTI or changing beta
        if (self.nchain > 1) and (self.MSTI or self.beta_step != 0.0):
            raise RuntimeError(
                "Parallel tempering (MPI>1) is incompatible with MSTI or continually changing beta. "
                "Run single-rank when MSTI=True or beta_step!=0."
            )

        # Mkralj: pre-run feasibility (no clipping): conservative worst-case (all accepts)
        beta0 = float(self.beta)
        if self.beta_step != 0.0:
            iter_hold = max(0, int(self.beta_hold_samples)) # hold is raw iterations
            remaining_iter = max(0, int(Niter) - iter_hold)

            if self.beta_step > 0.0:
                max_ok_accepts = int(np.floor((1.0 - beta0) / self.beta_step)) if self.beta_step > 0 else np.inf
            else:
                step = abs(self.beta_step)
                max_ok_accepts = int(np.floor(beta0 / step)) if step > 0 else np.inf

            if remaining_iter > max_ok_accepts:
                raise RuntimeError(
                    f"Given beta={beta0:.6f}, beta_step={self.beta_step:.3g}, thin={self.thin}, "
                    f"and beta_hold_samples={self.beta_hold_samples}, beta could leave [0,1] "
                    f"(needs ≤{max_ok_accepts} accepts; up to {remaining_iter} proposals after hold)."
                )


        # compute lnprob for initial point in chain... if resuming, just start with first point in chain
    ...
    def PTMCMCOneStep(
        ...
        if self.resume and self.resumeLength > 0 and iter < self.resumeLength * self.thin:
            ...
            if diff > rand_log:
                ...
                # Step beta only after the hold (counted in iterations), never clip
                if hasattr(self, "beta_step") and self.beta_step != 0.0:  # Only do this if beta stepping is enabled
                    if iter > getattr(self, "beta_hold_samples", 0):  # Wait until the number of held iterations has passed
                        new_beta = self.beta + self.beta_step  # Compute the proposed next beta value
                        if (new_beta < 0.0) or (new_beta > 1.0):  # Check if the new beta would leave the valid [0,1] range
                            raise RuntimeError(  # Stop immediately if the run would exceed the allowed beta range
                                f"Beta update would leave [0,1]: beta={self.beta:.6f} -> {new_beta:.6f} "
                                f"(beta_step={self.beta_step:.3g})"
                            )
                        self.beta = float(new_beta)  # Update the sampler’s beta if the proposed value is valid

                        # Adjust the baseline posterior to reflect the new beta (no recompute needed)
                        if self.MSTI:
                            # lnprob1, lnprob2 are the full model posteriors at the current state
                            lnprob0 += (lnprob1 - lnprob2) * self.beta_step
                        else:
                            # Single-model annealing: lnpost = beta * lnlike + lnprior => d lnpost / d beta = lnlike
                            lnprob0 += lnlike0 * self.beta_step
    ...
    def _writeToFile(self, iter):
        ...
        for ind in range(self.ind_next_write, write_end):
            ...
            # write beta immediately after parameters
